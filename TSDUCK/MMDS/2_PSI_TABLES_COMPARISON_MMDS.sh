...
do
	...
	INPUT_TSP_PSI_FILE="/root/outputs/MMDS/PSI_output_parsed/$MPTS"
	INPUT_TSP_TSDATE_FILE="/root/outputs/MMDS/tsp_tsdate_output/$MPTS"
	
	if [[ "$MPTS_DIGITS" -gt 44 ]]
	then
		TSID=132
	else
		TSID=$(($MPTS_DIGITS+200))
	fi
	...
	then
		...
		do
			OTHER_MPTS="232.200.2.$OTHER_MPTS_DIGITS:5200"
			INPUT_TSP_PSI_FILE="/root/outputs/MMDS/PSI_output_parsed/$OTHER_MPTS"
			
			if [[ "$OTHER_MPTS_DIGITS" -gt 44 ]]
			then
				OTHER_TSID=132
			else
				OTHER_TSID=$(($OTHER_MPTS_DIGITS+200))
			fi
